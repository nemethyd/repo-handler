...
: "${DEBUG_LEVEL:=2}"                  # VERBOSITY: 0=silent, 1=minimal, 2=normal, 3=verbose, 4=debug
                                     ...
                                     # FEEDBACK CONTROL: always, timed, never
                                     # Usage: log <MESSAGE_TYPE> <VERBOSITY_LEVEL> [<FEEDBACK_CONTROL>] "message"
: "${MAX_PACKAGES:=0}"
...
: "${BATCH_FEEDBACK_SILENCE_SECONDS:=15}"  # Suppress batch messages if any feedback occurred within this time

# Smart feedback control - time-based message suppression
LAST_FEEDBACK_TIME=0                       # Track when last feedback message was shown
# Log directory
...
# Smart feedback control function
function update_feedback_time() {
...
function adaptive_tune_performance() {
    ...
    # Fix efficiency calculation: convert to packages per second * 10 for meaningful integer arithmetic
    # total_time is in milliseconds, so divide by 1000 to get seconds, then multiply by 10 for precision
    local current_efficiency=$((total_packages * 10000 / total_time))  # packages per second * 10
    ...
    # Adjust EFFICIENCY_THRESHOLD to match new scale (multiply by 10)
    local efficiency_threshold=$((EFFICIENCY_THRESHOLD * 10))
                ...
                while [[ $retry_count -lt $max_retries ]] && [[ $fetch_success == false ]]; do
                    ...
                    if timeout "$DNF_TIMEOUT_SECONDS" dnf repoquery -y --arch=x86_64,noarch --disablerepo="*" --enablerepo="$repo" --qf "%{name}|%{epoch}|%{version}|%{release}|%{arch}" 2>>"$MYREPO_ERR_FILE" > "${cache_dir}/${repo}.cache.tmp"; then
                        ...
                        if [[ -n "${repo_versions[$repo]}" ]]; then
                            ...
                            log INFO 2 "Cached metadata for $repo ($(echo "$repo_data" | wc -l) packages) in ${repo_duration}s"
                        fi
...
function get_package_status() {
    ...
    [[ "${DEBUG_LEVEL:-0}" -ge 1 ]] && log DEBUG 3 "Checking package status: repo=$repo_name name=$package_name epoch=$epoch version=$package_version release=$package_release arch=$package_arch" >&2
    ...
    for rpm_file in "$repo_path"/"${package_name}"-*."$package_arch".rpm; do
        [[ "${DEBUG_LEVEL:-0}" -ge 2 ]] && log DEBUG 3 "Examining RPM file: $rpm_file" >&2
        ...
        [[ "${DEBUG_LEVEL:-0}" -ge 2 ]] && log DEBUG 3 "RPM details: name=$rpm_name epoch=$rpm_epoch version=$rpm_version release=$rpm_release arch=$rpm_arch" >&2
        [[ "${DEBUG_LEVEL:-0}" -ge 2 ]] && log DEBUG 3 "Comparing with: name=$package_name epoch=$epoch version=$package_version release=$package_release arch=$package_arch" >&2
        ...
        if [[ "$package_name" == "$rpm_name" \
           ...
           && "$package_arch" == "$rpm_arch" ]]; then
            [[ "${DEBUG_LEVEL:-0}" -ge 2 ]] && log DEBUG 3 "Found exact match!" >&2
            found_exact=1
            break
        elif [[ "$package_name" == "$rpm_name" \
              && "$package_arch" == "$rpm_arch" ]]; then
            [[ "${DEBUG_LEVEL:-0}" -ge 2 ]] && log DEBUG 3 "Found name/arch match but different version/release/epoch" >&2
            found_other=1
        else
            found_existing=1
        fi
    ...
    if ((status_duration > 500)); then
        log DEBUG 3 "$(align_repo_name "$repo_name"): Slow package status check for $package_name took ${status_duration}ms" >&2
    fi
...
function log() {
    ...
    local feedback_control="always"  # default value
    local message
    local color=""
    
    # Check if third parameter is a feedback control keyword
    if [[ "$3" =~ ^(always|timed|never)$ ]]; then
        feedback_control="$3"
        message="$4"
        color="${5:-}" # optional ANSI color for console
    else
        # Third parameter is the message (backward compatibility)
        message="$3"
        color="${4:-}" # optional ANSI color for console
    fi
    ...
    # Apply feedback control logic
    case "$feedback_control" in
        "never")
            return  # Never show this message
            ;;
        "timed")
            # Only show if sufficient time has passed since last feedback
            local current_time
            current_time=$(date +%s)
            local silence_duration=$((current_time - LAST_FEEDBACK_TIME))
            if [[ $silence_duration -lt $BATCH_FEEDBACK_SILENCE_SECONDS ]]; then
                return  # Suppress message - not enough silence time
            fi
            ;;
        "always")
            ...
            ;;
        *)
            # Unknown control type, default to always
            ;;
    esac

    # Update feedback time for user-visible messages (INFO, WARN, ERROR, SUCCESS)
    if [[ "$message_type" =~ ^(ERROR|WARN|INFO|SUCCESS)$ ]]; then
        update_feedback_time
    fi
...
function process_batch() {
    ...
    if ((${#batch_packages[@]} > 0)); then
        ...
        # Show batch processing message with timed feedback control
        log INFO 2 "timed" "Processing batch of ${#batch_packages[@]} packages..."
        ...
        # Show batch completion based on silence period and batch size
        if [[ ${#batch_packages[@]} -ge 25 ]]; then
            log INFO 2 "timed" "Batch completed: ${#batch_packages[@]} packages in ${batch_duration}ms (${packages_per_sec} pkg/sec)"
        elif [[ ${#batch_packages[@]} -ge 10 ]]; then
            log INFO 2 "timed" "Processed ${#batch_packages[@]} packages (${packages_per_sec} pkg/sec)"
...
function traverse_all_repos() {
    if ((SYNC_ONLY == 0)); then
        ...
        # Fetch installed packages list with repository information using DNF
        # Note: We need DNF here to get correct repository information, RPM only shows @System
        # However, if DNF fails, we'll fall back to RPM + repository detection
        if [[ -n "$NAME_FILTER" ]]; then
            log INFO 2 "Fetching list of installed packages (filtered by name pattern: $NAME_FILTER)..."
            log INFO 2 "Using DNF to get repository information (with RPM fallback if needed)..."
            local filter_fetch_start
            filter_fetch_start=$(date +%s)
            
            # Try DNF repoquery first with timeout
            if timeout 60 dnf repoquery --installed --qf "%{name}|%{epoch}|%{version}|%{release}|%{arch}|%{ui_from_repo}" 2>>"$MYREPO_ERR_FILE" | grep -E "^[^|]*${NAME_FILTER}[^|]*\|" >"$INSTALLED_PACKAGES_FILE"; then
                ...
                log INFO 2 "Found $package_count installed packages matching filter '$NAME_FILTER' in ${filter_fetch_duration}s (DNF query)"
            else
                log WARN 1 "DNF repoquery failed or timed out, falling back to RPM + repository detection..."
                # Fallback to RPM query
                if rpm -qa --queryformat '%{name}|%{epoch}|%{version}|%{release}|%{arch}|@System\n' 2>>"$MYREPO_ERR_FILE" | grep -E "^[^|]*${NAME_FILTER}[^|]*\|" >"$INSTALLED_PACKAGES_FILE"; then
                    local filter_fetch_end
                    filter_fetch_end=$(date +%s)
                    local filter_fetch_duration=$((filter_fetch_end - filter_fetch_start))
                    local package_count
                    package_count=$(wc -l < "$INSTALLED_PACKAGES_FILE")
                    log INFO 2 "Found $package_count installed packages matching filter '$NAME_FILTER' in ${filter_fetch_duration}s (RPM fallback)"
                else
                    log ERROR 0 "Both DNF and RPM queries failed while fetching filtered installed packages list."
                    exit 1
                fi
            fi
        else
            ...
            # Try DNF repoquery first with timeout
            if timeout 60 dnf repoquery --installed --qf "%{name}|%{epoch}|%{version}|%{release}|%{arch}|%{ui_from_repo}" >"$INSTALLED_PACKAGES_FILE" 2>>"$MYREPO_ERR_FILE"; then
                local package_fetch_end
                package_fetch_end=$(date +%s)
                local package_fetch_duration=$((package_fetch_end - package_fetch_start))
                local package_count
                package_count=$(wc -l < "$INSTALLED_PACKAGES_FILE")
                log INFO 2 "Fetched $package_count installed packages in ${package_fetch_duration}s (DNF query)"
            else
                log WARN 1 "DNF repoquery failed or timed out, falling back to RPM + repository detection..."
                # Fallback to RPM query
                if rpm -qa --queryformat '%{name}|%{epoch}|%{version}|%{release}|%{arch}|@System\n' >"$INSTALLED_PACKAGES_FILE" 2>>"$MYREPO_ERR_FILE"; then
                    local package_fetch_end
                    package_fetch_end=$(date +%s)
                    local package_fetch_duration=$((package_fetch_end - package_fetch_start))
                    local package_count
                    package_count=$(wc -l < "$INSTALLED_PACKAGES_FILE")
                    log INFO 2 "Fetched $package_count installed packages in ${package_fetch_duration}s (RPM fallback)"
                else
                    log ERROR 0 "Both DNF and RPM queries failed while fetching installed packages list."
                    exit 1
                fi
            fi
        fi
        ...
        log INFO 2 "Analyzing ${#package_lines[@]} installed packages for repository updates..."
        log DEBUG 3 "Building batches of $BATCH_SIZE packages for parallel processing..."
        package_counter=0
        local packages_processed=0  # Count of packages actually being processed (not filtered out)
        batch_packages=()
        ...
        for line in "${package_lines[@]}"; do
            ...
            # Increment counter for ALL packages processed (before filtering)
            ((package_counter++))
            
            # Progress feedback based on configurable intervals - show progress for ALL packages
            ...
            if ((current_main_time - last_main_feedback_time >= PROGRESS_FEEDBACK_SECONDS)) || ((package_counter % PROGRESS_FEEDBACK_PACKAGES == 0)); then
                ...
                if [[ $MAX_PACKAGES -gt 0 ]]; then
                    log INFO 2 "timed" "Progress: $package_counter/${#package_lines[@]} analyzed, $packages_processed/$MAX_PACKAGES processed ($rate pkg/min, batch: ${#batch_packages[@]})"
                else
                    log INFO 2 "timed" "Progress: $package_counter/${#package_lines[@]} analyzed, $packages_processed processed ($rate pkg/min, batch: ${#batch_packages[@]})"
                fi
                [[ $DEBUG_LEVEL -ge 3 ]] && log DEBUG 3 "Detailed progress: batch size: ${#batch_packages[@]}, elapsed: ${elapsed_main}s"
            ...
            if [[ -n "$repo_path" ]]; then
                ...
                # Check MAX_PACKAGES limit before adding the package
                if ((MAX_PACKAGES > 0 && packages_processed + ${#batch_packages[@]} >= MAX_PACKAGES)); then
                    log INFO 2 "Reached maximum package limit ($MAX_PACKAGES packages processed)"
                    break
                fi
                
                # Add package to batch (don't increment packages_processed here - do it when actually processing)
                ...
                # Check if we've reached MAX_PACKAGES limit immediately after adding to batch
                if ((MAX_PACKAGES > 0 && packages_processed + ${#batch_packages[@]} >= MAX_PACKAGES)); then
                    # Process limited batch immediately
                    local packages_left=$((MAX_PACKAGES - packages_processed))
                    local packages_to_process=("${batch_packages[@]:0:$packages_left}")
                    log INFO 2 "MAX_PACKAGES limit reached! Limiting batch to $packages_left packages due to MAX_PACKAGES=$MAX_PACKAGES"
                    log INFO 2 "timed" "Processing batch of ${#packages_to_process[@]} packages..."
                    process_batch "${packages_to_process[@]}"
                    packages_processed=$((packages_processed + ${#packages_to_process[@]}))
                    batch_packages=()
                    log INFO 2 "Breaking from main loop due to MAX_PACKAGES limit"
                    break
                fi
            else
                continue
            ...
            # Process batch when full OR when we need to respect MAX_PACKAGES limit  
            local should_process_batch=0
            if ((${#batch_packages[@]} >= BATCH_SIZE)); then
                should_process_batch=1
            elif ((MAX_PACKAGES > 0 && packages_processed + ${#batch_packages[@]} >= MAX_PACKAGES)); then
                should_process_batch=1
            fi
            
            if ((should_process_batch)); then
                # If we have a MAX_PACKAGES limit, only process what we're allowed to
                local packages_to_process=("${batch_packages[@]}")
                local batch_size=${#batch_packages[@]}
                
                if ((MAX_PACKAGES > 0)); then
                    local packages_left=$((MAX_PACKAGES - packages_processed))
                    if ((packages_left <= 0)); then
                        # Already reached limit, don't process anything
                        break
                    elif ((packages_left < batch_size)); then
                        packages_to_process=("${batch_packages[@]:0:$packages_left}")
                        log INFO 2 "Limiting batch to $packages_left packages due to MAX_PACKAGES=$MAX_PACKAGES"
                    fi
                fi
                ...
                # Update packages_processed to reflect what was actually processed
                packages_processed=$((packages_processed + ${#packages_to_process[@]}))
                batch_packages=()
                
                # Break if we've reached the package limit
                if ((MAX_PACKAGES > 0 && packages_processed >= MAX_PACKAGES)); then
                    break
                fi
            fi
        done
        if ((${#batch_packages[@]} > 0)) && ((MAX_PACKAGES == 0 || packages_processed < MAX_PACKAGES)); then
            # If we have a MAX_PACKAGES limit, only process what we're allowed to
            local packages_to_process=("${batch_packages[@]}")
            local batch_size=${#batch_packages[@]}
            ...
            if ((MAX_PACKAGES > 0)); then
                local packages_left=$((MAX_PACKAGES - packages_processed))
                if ((packages_left < batch_size)); then
                    packages_to_process=("${batch_packages[@]:0:$packages_left}")
                    log INFO 2 "Limiting final batch to $packages_left packages due to MAX_PACKAGES=$MAX_PACKAGES"
                fi
            fi
            
            log INFO 2 "timed" "Processing final batch of ${#packages_to_process[@]} packages..."
            process_batch "${packages_to_process[@]}"
        fi
        wait
